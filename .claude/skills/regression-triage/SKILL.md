...
```
https://<jenkins.url>/job/kiali/job/test-jobs/job/ossmc-cypress-tests/5772/
```

**Invalid (job root — reject this):**
```
https://<jenkins.url>/job/kiali/job/test-jobs/job/ossmc-cypress-tests/
```
...
OSSMC has two Cypress test directories:
- **OSSMC-specific**: `plugin/cypress/integration/openshift/featureFiles/` — fixes go here or in OSSMC
- **Vendored from kiali/kiali**: `plugin/cypress/integration/kiali/featureFiles/` — fixes belong in kiali/kiali (see note below)

For each failing scenario, search both directories:

```bash
grep -rl "<scenario name fragment>" plugin/cypress/integration/openshift/featureFiles/ plugin/cypress/integration/kiali/featureFiles/
```
...
awk '/^\s+@/ { buf=buf" "$0; next } /Scenario.*<scenario name fragment>/ { print buf; exit } { buf="" }' \
  plugin/cypress/integration/<openshift|kiali>/featureFiles/<file>.feature
```
...
If the scenario is found under `plugin/cypress/integration/kiali/featureFiles/`:
- Include in the handoff block: `Fix target: kiali/kiali`
- **Do not** send this handoff block to `/regression-fix` in OSSMC
- Instead, direct the user: "This scenario is from vendored Kiali tests. File the issue against `kiali/kiali` and run `/regression-fix` from the kiali/kiali repo."
...
3. If user picks combined → emit a **grouped handoff block** (see contract). If separate → emit individual blocks as normal.
...
gh issue list --repo kiali/openshift-servicemesh-plugin --search "<scenario fragment>" --state open
```

For vendored-test scenarios, also check the upstream repo:
...
- For blocks with `Fix target: kiali/kiali`, direct the user to the kiali/kiali repo

## Environment context

| Environment | Common failure patterns |
|-------------|------------------------|
| Jenkins / OCP (nightly default) | Auth timeouts, `CYPRESS_PASSWD` issues, slow cluster response, OSSMC console proxy errors |
