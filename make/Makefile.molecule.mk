...
.prepare-force-molecule-build:
	@$(eval FORCE_MOLECULE_BUILD ?= $(shell ${DORP} inspect ossmconsole-molecule:latest > /dev/null 2>&1 || echo "true"))
...
molecule-build: .prepare-force-molecule-build
	@if [ "${FORCE_MOLECULE_BUILD}" == "true" ]; then ${DORP} build --no-cache -t ossmconsole-molecule:latest ${OPERATOR_DIR}/molecule/docker; else echo "Will not rebuild ossmconsole-molecule image."; fi
