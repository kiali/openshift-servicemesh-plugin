...
const KialiDetailContent: FC<{ name: string; namespace: string }> = ({ name, namespace }) => {
  ...
  React.useEffect(() => {
    if (!pendingIntent || ossmConsoleStatusUnknown || !resource) return;
    if (promoted === (pendingIntent === 'promote')) {
  ...
  }, [name, ossmConsoleStatusUnknown, pendingIntent, promoted, resource, t]);
  ...
  const isDisabled = pendingIntent !== null || unavailableReason !== null;
            ...
            {(() => {
              const actionButton = promoted ? (
                <Button
                  ...
                  isLoading={pendingIntent === 'demote'}
                  onClick={() => void handleDemote()}
                  variant="warning"
                >
                  {pendingIntent === 'demote' ? t('Disconnecting…') : t('Disconnect')}
                </Button>
              ) : (
                <Button
                  isAriaDisabled={isDisabled}
                  isLoading={pendingIntent === 'promote'}
                  onClick={() => void handlePromote()}
                  variant="primary"
                >
                  {pendingIntent === 'promote' ? t('Connecting…') : t('Connect')}
                </Button>
              );
