...
const KialisPage: FC = () => {
  ...
  React.useEffect(() => {
    if (!pendingAction || ossmConsoleStatusUnknown) return;
    const nowPromoted = isPromoted(activeOssmConsole, pendingAction.name, pendingAction.namespace);
    if (nowPromoted === (pendingAction.intent === 'promote')) {
  ...
  }, [activeOssmConsole, ossmConsoleStatusUnknown, pendingAction, t]);
  ...
  const rowData = useMemo<KialiRowData>(
    () => ({
      ...
      pendingKey: pendingAction?.key ?? null,
      routeHostMap
    }),
    [
      activeOssmConsole,
