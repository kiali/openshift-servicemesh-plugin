...
import { runLayout } from '../../../components/CytoscapeGraph/CytoscapeGraphUtils';
...
export class GraphFind extends React.Component<GraphFindProps, GraphFindState> {
  ...
  private handleHide = (
    ...
    if (
      ...
      runLayout(cy, this.props.layout, this.props.namespaceLayout).then(() => {
    ...
    switch (field.toLowerCase()) {
      ...
      default:
        ...
        if (field.startsWith('label:')) {
          return { target: 'node', selector: `[${toSafeCyFieldName(field)} ${op} "${val}"]` };
        }
  ...
  private parseUnaryFindExpression = (field: string, isNegation): ParsedExpression | undefined => {
    switch (field.toLowerCase()) {
      ...
      default:
        // special node operand
        if (field.startsWith('label:')) {
          const safeFieldName = toSafeCyFieldName(field);
