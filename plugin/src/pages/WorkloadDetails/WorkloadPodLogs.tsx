...
import { KialiIcon, defaultIconStyle } from '@kiali/core-ui';
...
export class WorkloadPodLogs extends React.Component<WorkloadPodLogsProps, WorkloadPodLogsState> {
  ...
  private gotoSpan = (span: TracingSpan) => {
    const link =
  ...
  private setLogLevel = (level: LogLevel) => {
    ...
    API.setPodEnvoyProxyLogLevel(this.props.namespace, pod.name, level, this.props.cluster)
      .then(_resp => {
        AlertUtils.addSuccess(`Successfully updated proxy log level to '${level}' for pod: ${pod.name}`);
      })
      .catch(error => {
        AlertUtils.addError('Unable to set proxy pod level', error);
      });
  };
  ...
  private fetchEntries = (
    ...
    const promises: Promise<AxiosResponse<PodLogs | TracingSpan[]>>[] = selectedContainers.map(c => {
      return API.getPodLogs(namespace, podName, c.name, maxLines, sinceTime, duration, c.isProxy, cluster);
    });
    if (showSpans) {
      ...
      promises.unshift(API.getWorkloadSpans(namespace, this.props.workload, params, this.props.cluster));
    }
    ...
    this.promises
      ...
      .then(responses => {
        ...
        if (showSpans) {
          const spans = showSpans ? (responses[0].data as TracingSpan[]) : ([] as TracingSpan[]);
