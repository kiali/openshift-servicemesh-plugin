...
import { getNamespaces, ActiveFiltersInfo, PromisesRegistry, CanaryUpgradeStatus, NamespaceInfo, OverviewCard, OverviewToolbar, nameFilter, DirectionType, OverviewDisplayMode, OutboundTrafficPolicy, ComponentStatus, OverviewType, sortFields, sortFunc, SortField } from '@kiali/core-ui';
import { fetchCanaryStatus, fetchIstioStatus, fetchHealth, fetchTLS, fetchOutboundTrafficPolicyMode, fetchIstiodResourceThresholds, fetchValidations, fetchMetrics } from './OverviewCalls';
...
const OverviewPage = () => { 
    ...
    const load =  (filters: ActiveFiltersInfo = activeFilters, conf: KialiConfig = kialiConfig) => {   
        promises.cancelAll()
        promises
            .register('getNamespaces', getNamespaces(conf.kialiProxy))
            .then(namespacesResponse => {
                const nameFilters = filters.filters.filter(
                    f => f.category === nameFilter.category
                );
                ...
                fetchCanaryStatus(conf.kialiProxy, setCanaryStatus, setCanaryUpgrade)
                fetchIstioStatus(conf.kialiProxy, setComponentStatus)    

                fetchHealth(sortNamespaces, isAscending, sortFields[0], overviewType, setNamespaces, conf.server, conf.kialiProxy);
                fetchTLS(sortNamespaces, isAscending, sortFields[0], conf.meshTLSStatus.status, setNamespaces, conf.server, conf.kialiProxy);        
                fetchValidations(sortNamespaces,isAscending, sortFields[0],setNamespaces, conf.server, conf.kialiProxy);  
                fetchOutboundTrafficPolicyMode(setoutboundTrafficPolicy, conf.kialiProxy);
                fetchIstiodResourceThresholds(setIstiodResourceThresholds, conf.kialiProxy);
                if (displayMode !== OverviewDisplayMode.COMPACT) {
                    fetchMetrics(allNamespaces, duration, direction, setNamespaces, conf.server, conf.kialiProxy);
                }             
                setLoading(false); 
            })      
    }
    ...
    return (
        isLoading? <></> :         
            <>
                ...
                />                
                    <Grid>
                    {!isLoading && sortFunc(namespaces, sortFields[0], isAscending, kialiConfig.server).map((ns, i) => {
                        const isGreaterItem = ns.name === kialiConfig.server.istioNamespace &&
                        ...
                        return (
                            ...
                            >
                                <OverviewCard 
                                    ...
                                    overviewType={overviewType}
                                    certificatesInformationIndicators={kialiConfig.server.kialiFeatureFlags.certificatesInformationIndicators.enabled}
