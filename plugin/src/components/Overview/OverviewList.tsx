...
import {
    ...
    RowProps,
    TableColumn,
    TableData, useActiveColumns, VirtualizedTable
} from '@openshift-console/dynamic-plugin-sdk';
import { cellWidth , sortable, textCenter } from '@patternfly/react-table';
import { ActiveFiltersInfo, ActiveFilter, ControlPlaneBadge, ComponentStatus, Label, NamespaceInfo, NamespaceStatuses, NamespaceMTLSStatus, IstiodResourceThresholds, OverviewCardSparklineCharts, OverviewType, PFBadge, PFBadges, ValidationSummary, DirectionType } from '@kiali/core-ui';
...
const Row = ({ obj, activeColumnIDs }: RowProps<NamespaceInfo>, istioStatus: ComponentStatus[], filters: ActiveFiltersInfo, overviewType: OverviewType, duration: number, direction: DirectionType, istiodResourceThresholds: IstiodResourceThresholds,config: KialiConfig) => {
        ...
        <>
            ...
            <TableData id={columns[3].id} activeColumnIDs={activeColumnIDs}>
                {Object.entries(obj.labels).map(([key, value], i) => {
                    ...
                    const isExactlyLabelFilter = filters.filters.some(f =>
...
const IstioTable = ({
    ...
    return (
        <VirtualizedTable<NamespaceInfo>
            ...
            loadError={loadError}
            columns={columns}
            Row={(obj: RowProps<NamespaceInfo>) => Row(obj, istioStatus, filters, overviewType, duration, direction, istiodResourceThresholds, config)}
        />
    );
};
