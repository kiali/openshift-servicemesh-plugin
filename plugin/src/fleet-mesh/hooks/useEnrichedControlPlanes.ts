...
export function useEnrichedControlPlanes(
  ...
  const [errorSearchKey, setErrorSearchKey] = useState(0);
  // Only treat enrichment as incomplete on the first cycle. After initial enrichment
  // completes, subsequent search poll updates do not reset enrichmentLoaded so the
  // table never flashes a spinner — new CRs briefly appear as standalone (~1s) before
  // enrichment reveals their meshID and the next rebuild regroups them.
  ...
  const allCached =
    stableResults.length === 0 || stableResults.every(r => enrichmentCache.has(`${r.cluster}/${r.metadata?.name}`));
  const enrichmentLoaded = allCached || enrichmentCompleted;
  const activeError = errorSearchKey === searchKey ? error : null;
  ...
  return [enrichedPlanes, searchResults.length > 0 || enrichmentLoaded, enrichmentLoaded, activeError];
}
