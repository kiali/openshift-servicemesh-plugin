...
describe('OverviewPage', () => {
  ...
  it('shows an informational empty state when the OSSM-ACM addon is not installed', () => {
    mockDefaults({ mcmsError: new Error('Model does not exist') });
    render(<OverviewPage />);
    expect(
      screen.getByText(
    ...
    expect(screen.queryByText('Unable to load mesh data')).not.toBeInTheDocument();
  });

  it('does not warn about mesh issues when only the OSSM-ACM addon is missing', () => {
    mockDefaults({ mcmsError: new Error('Model does not exist') });
    render(<OverviewPage />);
    expect(screen.queryByText('Unable to load mesh data. Some issues may not be shown.')).not.toBeInTheDocument();
    expect(screen.getByText('No issues detected.')).toBeInTheDocument();
  });
