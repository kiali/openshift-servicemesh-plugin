...
// True only when the OSSMConsole CR sets spec.internal.techPreview: true. Gates the unsupported
// tech preview features (Istios and Kialis list pages and Fleet Service Mesh) off by default -- see the cross-cutting
// FLEET_MESH_AVAILABLE + OSSMC_INTERNAL_TECH_PREVIEW combination in fleet-mesh-extensions.ts and the
// per-item gating in lite-extensions.ts.
//
...
//
// Unlike KIALI_AVAILABLE/FLEET_MESH_AVAILABLE, this does not use probeWithRetry: that helper only
...
function useOssmcInternalTechPreviewFlag(setFlag: (flag: string, value: boolean) => void): void {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- one-shot fetch on mount only
  }, []);
}
