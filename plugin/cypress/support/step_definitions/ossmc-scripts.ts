...
Then('instruct the Kiali Operator to create a small OSSMConsole CR', () => {
    cy.exec('bash ./../hack/manage-ossmc.sh --install', { timeout: 60000 }).its('stdout').should('eq', 'ossmconsole.kiali.io/ossmconsole created');
});
...
Then('uninstallation scripts to clean OSSMC resources is successful', () => {
    cy.exec('bash ./../hack/manage-ossmc.sh --uninstall', { timeout: 60000 }).its('stdout').should('eq', 'ossmconsole.kiali.io "ossmconsole" deleted');
});