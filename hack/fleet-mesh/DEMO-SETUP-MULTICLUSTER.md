...
2. Installs the Sail/OSSM operator via OLM ManifestWork (shared across meshes on the same cluster)
...
The Fleet Service Mesh perspective is part of the combined OSSMC ConsolePlugin
...
> **Tip:** For iterative frontend development (live-reload with `make fleet-mesh-start`
> and a local Console via `make fleet-mesh-start-console`), see the "Frontend development"
...
The controller installs the Sail operator via ManifestWork, which requires the klusterlet
...
2. Install the Sail operator via OLM ManifestWork (shared across meshes on the same cluster)
...
Each MCM requires an Istio CR on every cluster in the mesh. The Istio CR tells the Sail
...
- Sail operator CSV in `Succeeded` phase on both clusters
...
# 11. Remove the Sail operator CSV and CRDs on both clusters
